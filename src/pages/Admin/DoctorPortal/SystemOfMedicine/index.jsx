      ...
      if (navigator.onLine) {
        const response = await put(
          BaseSetting.adminApiDomain +
            `/systemOfMedicine/updateSystemOfMedicine/${editData._id}`,
          inputs,
          headers
        );
    ...
    try {
      if (navigator.onLine) {
        const response = await post(
          BaseSetting.adminApiDomain +
            '/systemOfMedicine/addNewSystemOfMedicine',
          inputs,
          headers
        );
        // setApiData(response.data.data);
    ...
    try {
      if (navigator.onLine) {
        await axios
          .delete(
            BaseSetting.adminApiDomain +
              `/systemOfMedicine/removeSystemOfMedicine/${id}`,
            { headers }
          )
          .then(() => {
            getData();
    ...
    try {
      if (navigator.onLine) {
        const response = await get(
          BaseSetting.adminApiDomain + '/systemOfMedicine/getAll',
          headers
        );
        // setApiData(response.data.data);
            ...
            <div className="fixed inset-0 z-50 flex items-center overflow-x-hidden overflow-y-auto outline-none transjustify-center focus:outline-none">
              <div className="relative w-auto max-w-3xl mx-auto my-6">
                ...
                <div className="relative flex flex-col w-full bg-white border-0 rounded-lg shadow-lg outline-none focus:outline-none">
                  ...
                  <form>
                    <div className="relative flex-auto p-3">
                      <div className="grid grid-cols-1 gap-5 leading-relaxed">
                        <div className="flex flex-col ">
                          <div className="flex flex-row">
                            <input
                              className="w-full border py-2 px-1 rounded outline-none font-montserrat font-normal text-gray-400 text-[12px]"
...
export default SystemOfMedicine;
