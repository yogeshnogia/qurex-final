...
const Specialization = () => {
  ...
  const [specDec, setSpecDec] = useState('');
      ...
      if (navigator.onLine) {
        const response = await put(
          BaseSetting.adminApiDomain +
            `/specialization/updateSpecialization/${editData._id}`,
          inputs,
          headers
        );
    ...
    try {
      if (navigator.onLine) {
        const response = await post(
          BaseSetting.adminApiDomain + '/specialization/addNewSpecialization',
          inputs,
          headers
        );
        // setApiData(response.data.data);
    ...
    try {
      if (navigator.onLine) {
        await axios
          .delete(
            BaseSetting.adminApiDomain +
              `/specialization/removeSpecialization/${id}`,
            { headers }
          )
          .then((res) => {
            console.log(res);
  ...
  const columns = [
    ...
    },
    {
      ...
      selector: (row) => {
        return (
          <div>
            <img className="py-3" height={34} src={think} alt="" />
          </div>
        );
      },
    },
