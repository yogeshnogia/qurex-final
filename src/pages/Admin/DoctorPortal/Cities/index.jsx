...
const Cities = () => {
    ...
    try {
      if (navigator.onLine) {
        const response = await put(
          BaseSetting.adminApiDomain + `/city/updateCity/${editData._id}`,
          inputs,
          headers
        );
    ...
    try {
      if (navigator.onLine) {
        const response = await post(
          BaseSetting.adminApiDomain + '/city/addNewCity',
          inputs,
          headers
        );
        // setApiData(response.data.data);
  ...
  const deleteData = async (id) => {
    try {
      if (navigator.onLine) {
        await axios
          .delete(BaseSetting.adminApiDomain + `/city/removeCity/${id}`, {
            headers,
          })
          .then(() => {
            getData();
    ...
    try {
      if (navigator.onLine) {
        const response = await get(
          BaseSetting.adminApiDomain + '/city/getAll',
          headers
        );
  ...
  return (
    <div className="flex flex-col px-10 font-montserrat">
      <div className="py-6 font-semibold text-2xl text-[#636363]">Cities</div>
      <div className="flex flex-row">
        <div
